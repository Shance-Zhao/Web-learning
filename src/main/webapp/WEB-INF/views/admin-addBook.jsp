...
<script type="text/javascript">
	function createTable() {
		...
		var theader = "<table border='1'>"
			+"<tr><th width=100>Name</th>"+ "<th width=100>ISBN</th>"
			+ "<th width=100>Author</th>" + "<th width=100>Picture</th>"
			+ "<th width=100>Quantity</th>" + "<th width=100>Price/per</th>"
			+ "<th width=100>Sale Price</th>" + "<th width=100>Description</th></tr>";
		var tbody="";
		
		for (i = 0; i < num; i++) {
			tbody += "<tr><td><input name='name' id='name' size='10' required='required' /></td>"
					+ "<td><input type='text' name='isbn' id='isbn' size='10' required='required'/></td>"
					+ "<td><input type='text' name='author' id='author' size='10' required='required'/></td>"
...
<body>
					...
					<ul class="nav navbar-nav navbar-right">
						...
						<li class="dropdown"><a style="font-size: 2ex; color: #fff"
							...
							href="${contextPath}/admin/profile.htm">Account <span
								class="caret"></span>
						</a> <c:if test="${adm.getAdminName() != null}">
								<ul class="dropdown-menu">
									<li><a href="${contextPath}/admin/profile.htm"><c:out
												value="${adm.getAdminName()}" /></a></li>
	...
	<form method='post' action='${contextPath}/book/addsuccess'  enctype="multipart/form-data">
